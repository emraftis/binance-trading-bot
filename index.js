...
const getData = async function (symbol, timeframe) {
  ...
  const lengthOfTime = 60 * 60 * 24 * 30 * 1000;
  ...
  console.log("Current Average Price: " + currentAvgPrice);
  ...
  if (currentAvgPrice >= 1.03 * currentActualPrice) {
    console.log(
      `Action: Buy without time restriction at ${currentActualPrice}`
    ); //no time restriction, aka buy the dip!
    console.log(
      `Action: Set limit sell at ${
        currentAvgPrice * 1.015
      } for 55% of ETH purchased`
    );
    console.log(
      `Action: Set limit sell at ${
        currentAvgPrice * 1.03
      } for 35% of ETH purchased`
    );
    console.log(
      `Action: Set limit sell at ${
        currentAvgPrice * 1.05
      } for 10% of ETH purchased`
    );
  } else if (currentAvgPrice > currentActualPrice) {
    console.log(`Action: Buy with time restriction at ${currentActualPrice}`); //
    console.log(
      `Action: Set limit sell at ${
        currentAvgPrice * 1.015
      } for 55% of ETH purchased`
    ...
    console.log("Action: Market sell 20% of ETH holdings with timer");
  } else if (currentActualPrice > currentAvgPrice * 1.1) {
    console.log("Action: Market sell 10% of ETH holdings with timer");
  } else {
    console.log("Action: HODL");
  }
  ...
  return console.log("--Function Ended--");
...
setInterval(getData, 30000, "ETH", "12h");
